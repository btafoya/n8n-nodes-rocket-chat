...
import { rcApiRequestAllItems, rcApiRequestForm, resolveRoomId } from '../transport/GenericFunctions';

export class RocketChatFile implements INodeType {
	...
	async execute(this: IExecuteFunctions) {
		...
		for (let i = 0; i < items.length; i++) {
			const targetMode = this.getNodeParameter('targetMode', i) as string;
			const freeRoom = this.getNodeParameter('room', i, '') as string;
			const pickedRoom = this.getNodeParameter('roomPicker', i, '') as string;
			...
			const rid = await resolveRoomId.call(this, roomTarget);
			...
			const res = await rcApiRequestForm.call(this, `/api/v1/rooms.upload/${encodeURIComponent(rid)}`, formData);
