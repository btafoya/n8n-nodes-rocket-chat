    ...
    async execute() {
        ...
        for (let i = 0; i < items.length; i++) {
            ...
            if (operation === 'post' || operation === 'send') {
                ...
                const res = await GenericFunctions_1.rcApiRequest.call(this, 'POST', endpoint, body);
            ...
            if (operation === 'update') {
                ...
                const res = await GenericFunctions_1.rcApiRequest.call(this, 'POST', '/api/v1/chat.update', {
                    roomId: rid,
                    msgId,
                    text: newText,
                });
            ...
            if (operation === 'delete') {
                const rid = await GenericFunctions_1.resolveTargetRid.call(this, i);
                const msgId = this.getNodeParameter('msgId', i);
                const res = await GenericFunctions_1.rcApiRequest.call(this, 'POST', '/api/v1/chat.delete', {
                    roomId: rid,
                    msgId,
                });
                returnData.push({ json: res });
            ...
            if (operation === 'openDm') {
                const username = this.getNodeParameter('dmUsername', i);
                const res = await GenericFunctions_1.rcApiRequest.call(this, 'POST', '/api/v1/im.open', { username });
                returnData.push({ json: res });
            }
            if (operation === 'sendDm') {
                const username = this.getNodeParameter('dmUsername', i);
                ...
                const tmid = this.getNodeParameter('tmid', i, '');
                const additional = this.getNodeParameter('additionalFields', i, {});
                const dm = await GenericFunctions_1.rcApiRequest.call(this, 'POST', '/api/v1/im.open', { username });
                ...
                if (additional.attachments)
                    body.attachments = JSON.parse(additional.attachments);
                const res = await GenericFunctions_1.rcApiRequest.call(this, 'POST', '/api/v1/chat.postMessage', body);
                returnData.push({ json: res });
            }
        }
